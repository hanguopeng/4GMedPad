...
function playmusic () {
  ...
  $api.byId('audioName').innerHTML=chooseName;
...
//切换音频
function switchmusic(condition) {
  var button = $api.byId("playImg");
  if(chooseName==""){
    api.toast({
        msg: '请选择一个音频',
        duration: 2000,
  ...
  if(condition === 0){ //condition=0 鼠标点击图片
    ...
    if (click == 2) {
      ...
      playmusic();
      //console.log("condition = 0 click="+click);
    }else {
      // 关闭
...
apiready = function() {
  ...
  playbg.style.height = api.frameHeight + 'px';
...
function  pagerLocalSearch(){
  ...
  var userId = $api.getStorage(storageKey.userId);
  if(curNum == 0){
    var sql = "SELECT * FROM " + cmcdb.resTable + " where person_id='"+ person.id +"' and res_type='0' and user_id='"+ userId +"' order by id desc limit " + curNum + "," + pageSize;
  }else{
    var sql = "SELECT * FROM " + cmcdb.resTable + " where id <'"+ dataId +"' and person_id='"+ person.id +"' and res_type='0' and user_id='"+ userId +"' order by id desc limit 0," + pageSize;
  }
         ...
         UIListMeeting.getDataByIndex({
           ...
           if (ret1) {
               ...
               if(btnType == 0){
                 ...
                 if(ret.btnIndex == 0){
                   //上传
                   uploadLocalAudio(ret.index);
                 }else if(ret.btnIndex == 1){
                   //本地删除
                   deleteLocalAudio(ret.index);
                 }
               }else if(btnType == 1){
                 //本地已上传
                 if(ret.btnIndex == 0){
                 ...
                 if(ret.btnIndex == 0){
                   //下载
                   downloadCloudAudio(ret.index);
                 }else if(ret.btnIndex == 1){
                   //云端删除
                   deleteCloudAudio(ret.index);
                 }
               }else if(btnType == 3){
                 //云端已下载
                 if(ret.btnIndex == 0){
...
//云端下载 0003
function downloadCloudAudio(index){
  api.showProgress({
    title:'',
    text: '下载中...'
  });
  UIListMeeting.getDataByIndex({
    ...
    var userId = $api.getStorage(storageKey.userId);
    var person = $api.getStorage(storageKey.currentPerson);
    var file = 'fs://'+userId+"/"+ ret.data.name +".amr";
    api.download({
...
function delAudioCloudAndLocal(ret,index){
  common.post({
    ...
    success:function(r){
      var userId = $api.getStorage(storageKey.userId);
      fs.remove({
          path: 'fs://'+userId+"/"+ ret.data.name +".amr"
      }, function(ret1, err) {
          if (ret1.status) {
            UIListMeeting.deleteItem({
            index: index
...
//本地上传音频
//0002
function uploadLocalAudio(index){
  api.showProgress({
    title:'',
    text: '音频上传中...'
  });
...
//本地删除音频
function deleteLocalAudio(index){
  //console.log("deleteLocalAudio");
  api.confirm({
      title: '提示',
      msg: '您确定删除该录音吗？',
      buttons: ['确定', '取消']
...
/*var searchDetails = function(){
    ...
    var recodTime = $api.val($api.byId('dateRange'));
    var recoder = $api.val($api.byId('recoder'));
    var person = $api.getStorage(storageKey.currentPerson);
    var url = config.accessoryQueryUrl+"?localId="+dataId+"&type="+accessoryType.audio+"&patientId="+person.id+"&limit="+pageSize+"&page="+cloudPage;
    if("" != recodTime){
        url = url + "&recordTime="+recodTime;
    }
    if("" != recoder){
        url = url + "&recordPerson="+recoder;
    }
    common.get({
        url:url,
        isLoading:true,
